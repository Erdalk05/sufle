...
| 53 | 2026-08-15 | **🔴 Erdal bildirdi: alt kumandalar üst üste biniyor** — kök neden BENİM düzeltmemdi | ekran görüntüsü sayıya çevrildi: hız hapı ↔ durum satırı **7 px**, durum satırı ↔ çubuk **9 px** · kök neden **T34**: çubuğu dar telefonlara sığdırırken 95 px'e çıkardım, üstteki katmanlar hâlâ sabit 86/96 px'e dayanıyordu · **aynı kusur v8.6'da da olmuş** ve düzeltmesi piksel değeriyle kilitlendiği için sessizce geri gelmiş · yapısal çözüm: iki katman çubuğun üstünde tek **dikey yığın**, konum çubuğun hesaplanan yüksekliğinden · 4 genişlikte **çakışma 0** · `ekran.py` artık her karede **çakışan kumanda sayıyor** (ayırt ettiği kanıtlandı: 0 → 5) · v9.11 | ✅ 9/9 YEŞİL |
| 54 | 2026-08-15 | **🔴 Erdal: "iOS'ta sesle takip çalışmıyor"** — sebep aranmadan önce SESSİZLİK kırıldı | şüpheli üç yolun üçü de iOS'ta ZATEN KAPALI çıktı (FX `!IS_WK`, ölçer `!IS_WK`, vad varsayılan false) → mikrofonu Web Audio'ya bağlayan bizim kodumuz değil · geriye **kamera mikrofonu tutuyor** kalıyor (iOS ses oturumu tekil) · **sebep tahmin edilmedi, ELEME yapıldı:** 6 sn sonuç gelmezse nöbetçi kamera açıksa onu söylüyor ve kamerasız kipi öneriyor, öz-test de kamerayı ayırt ediyor · 4 kanıtlı bozma (**96**) · **4676 test** | ✅ 9/9 YEŞİL |
| 55 | 2026-08-15 | **🔴 Erdal: "yapay zekâ tasarımı gibi duruyor, insan eli değsin"** — öznel şikâyet dört ÖLÇÜLEBİLİR nedene indi | ① derinlik yok: 6000 satırda **yalnız 4 gölge, 4 basış geri bildirimi** → yükselti jetonları (üstten ışık: üst kenarda 1 px aydınlık + alta gölge), **çizilmiş arayüzde 196 gölgeli öğe** · ② **`jetonlar.css` ÖLÜ AYARMIŞ**: telefon yüzey renklerini elle yazıyordu (#141418/#1e1e24/#2a2a32), jeton dosyasını değiştirmek ekranda hiçbir şeyi değiştirmiyordu → dördü de çekirdeğe bağlandı, nötr griden mavi-grafite kaydı (zemin `#0A0C11`; sufle metninin kendi zemini saf siyah KALDI — kamera için doğru) · ③ **236 süs emojisi** etiketlerden alındı (sözlük 236 · işaretleme 106 · yer tutucu 3); sınır yazıldı: `mesajlar.js` DURUM emojileri (✅⚠️⛔) duruyor · Mac'te de aynısı yapılmasaydı iki platform ayrışırdı (tests/122 32 uyuşmazlık yakaladı) · ④ altı öğeli seçicide son düğme **tek başına tam genişlik** kaplıyordu → ızgara, **yetim satır 0** · kaydırıcı yerelden bize geçti ve dolgu geri geldi (min≠0 tuzağı: hız 60'tan başlıyor, ham değerle çubuk hiç boşalmazdı) · sayı artık yeşil değil (yeşil = TEK asıl eylem) · `tests/143` **44 iddia** · **4720 test** | ✅ 9/9 YEŞİL |
| 56 | 2026-08-15 | **30 kategorilik rekabet analizi ÖLÇÜLDÜ** — tahmin puanları koda bağlandı | analiz SUFLE sütununu tahminle puanlamıştı: **53,6/100, 6. sıra** · aynı rubrikte her puan kodda ölçüldü → **63,0/100, 4. sıra** (PromptSmart 56,0'ın üstü, BIGVU 65,3'ün altı) ve **+9,4 puanın tamamı zaten yayında olan özelliklerden** geldi, tek satır yeni kod yazılmadı · 11 kategori düzeltildi, her biri kanıtlı (docx içe aktarma, 4K+MP4, budama+kompozit, OBS tarayıcı kaynağı `?obs=1`, tuş öğretme, ASR'siz altyazı) · **4 sıfır DEĞİŞMEDİ** — AI üçlüsü, bulut, kurulu taban, takım: ölçüm analizi doğruladı · **asıl bulgu:** sıfır alan 6 kategorinin ağırlığı 17, yani tümün %18'i ve altısı da **sunucu/hesap/pazar** gerektiriyor → **sunucusuz tavan 81,9**, bugün 63,0, yani istemcide hâlâ **18,9 puan** var (liderin hemen altı) · `FIYATLANDIRMA.md` ile aynı sonuca bağımsız yoldan varıldı · `tests/144` **35 iddia** aritmetiği yeniden hesaplıyor · 7 kanıtlı bozma (**103**) · **4759 test** | ✅ 9/9 YEŞİL |
