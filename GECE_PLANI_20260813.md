...
| # | Görev | Ö |
|---|---|---|
| D1 | ⛔ Uzun senaryoda kaybolma oranı — **ÖLÇÜLDÜ, KAYBOLMUYOR**. 5000 kelime, %5 atlama, %8 tanıma hatası, %64 benzersiz kelime: işaretçi sona **4999/5000** ulaşıyor, geniş arama 4734 adımda yalnız **3 kez** gerekiyor, 600 kelime ileri sıçrayan kullanıcı **17 kelimede** yakalanıyor. **İki kez yanlış ölçtüm**: (1) harness her adımda kelimeyi iki kez gönderiyordu; (2) düzeltince çıkan “ortalama sapma 52” rakamını kusur sanıp geniş modda tek-kelime kabulünü kapatan bir yama yazdım — yama sayıları hiç değiştirmedi, çünkü sebep o değildi; o sapmalar kurtarma öncesi **geçici gecikme**ymiş. Yamayı geri aldım. **Kalan gerçek bulgu**: geriye dönüş (cümleyi baştan okumak) ileri sıçramadan **4 kat yavaş** yakalanıyor (~70 kelime) — `WIN_BACK`(8) < `WIN_FWD`(26) ve geride kalan dolgu kelimeleri geniş aramanın sayacını sıfırlıyor. Ayrı iş olarak D11'e alındı. Test 65 (19 iddia) | P1 |
| D11 | ⬜ **YENİ (D1 ölçümünden)** — sesle takipte GERİYE dönüş ~70 kelimede yakalanıyor, ileri sıçrama ~17'de. Cümleyi baştan okumak sık yapılan bir şey; 140 wpm'de bu ~30 saniye. Sebep ölçüldü: `WIN_BACK` küçük + geride kalan dolgu kelimeleri zayıf eşleşme üretip geniş aramanın 1,2 sn sayacını sürekli sıfırlıyor | P1 |
